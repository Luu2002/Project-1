...
```{r}
boxplot(survey$Height, main = "Student Height Boxplot",
        xlab = "Height(cm)",
        horizontal = TRUE,
        notch = TRUE)
```
...
By drawing the regression lines to have a clear picture of the analysis, without any significant difference between male and female students. Since female students are shorter than male students, and the height of male students are varies more than female students.\
...
The graph below contains different significant data, each dot represents each student's data, and they are colored in blue and red color to distinguish between gender (with blue dots represent male students and red dots represent female students).\ 
...
ggplot(survey, aes(x = Wr.Hnd, y = NW.Hnd)) +
  geom_point(aes(color = Sex), size = 2) +
  geom_smooth(aes(color = Sex), method = "lm") +
  labs(title = "Span of Writing hands Versus Span of non-writing hands among male and female",
